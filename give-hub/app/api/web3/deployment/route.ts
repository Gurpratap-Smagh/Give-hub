...
export async function GET() {
  try {
    ...
    const systemContractFromEnv = process.env.NEXT_PUBLIC_SYSTEM_CONTRACT_ADDRESS
    if (addrFromEnv && chainFromEnv && wzetaFromEnv) {
      return NextResponse.json({ address: addrFromEnv, chainId: Number(chainFromEnv), wzeta: wzetaFromEnv, systemContract: systemContractFromEnv })
    }
    ...
    if (!fs.existsSync(deploymentsPath)) {
      return NextResponse.json({ error: 'Deployment file not found. Set NEXT_PUBLIC_CROSSCHAIN_CONTRACT (or NEXT_PUBLIC_GIVEHUB_CONTRACT_ADDRESS / NEXT_PUBLIC_CROWDFUND_ADDRESS), NEXT_PUBLIC_ZETA_CHAIN_ID (or NEXT_PUBLIC_CHAIN_ID) and NEXT_PUBLIC_WZETA_ADDRESS (or NEXT_PUBLIC_WZETA).' }, { status: 404 })
    }
    ...
    const systemContract: string | undefined = json?.systemContracts?.systemContract
