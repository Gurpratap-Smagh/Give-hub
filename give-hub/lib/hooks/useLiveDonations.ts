...
export function useLiveDonations(
  ...
  // Polling-based event listening (fallback for RPC without eth_subscribe)
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (currentBlock > lastBlockNumber) {
          const fromBlock = lastBlockNumber > 0 ? lastBlockNumber + 1 : currentBlock - 100; // Look back 100 blocks on first run
          
          // Query donation events
          ...
          try {
            ...
            for (const log of donationLogs) {
              try {
                ...
                console.log('[useLiveDonations] New donation:', decoded);
            ...
            for (const log of campaignLogs) {
              try {
                ...
                console.log('[useLiveDonations] New campaign:', decoded);
            ...
            console.error('[useLiveDonations] Failed to fetch logs:', logErr);
          }
    ...
    // Set up polling interval (every 3 seconds for faster detection)
